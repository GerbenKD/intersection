...
var Tool = new function() {
	...
	this.toString = function() {
		var info = { id : this.id, inputs: [], outputs: [] };
		for (var i=0; i<this.max_input_socket(); i++) {
			var inp = this.get_input(i);
			if (inp) {
				info.inputs[i] = inp[0].id+":"+inp[1];
			} else {
				info.inputs[i] = ".";
			}
		}
		for (var i=0; i<this.max_output_socket(); i++) {
			var outp = this.get_output(i);
			info.outputs[i] = outp ? outp.toString() : ".";
		}	
		return this.typename + JSON.stringify(info);
	}
...
var LCI_Tool = BasicTool.extend(function() {
	...
	this.typename = "LCI"; // intended for debugging
...
var LLI_Tool = BasicTool.extend(function() {
	...
	this.typename = "LLI"; // intended for debugging
