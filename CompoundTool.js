...
var CompoundTool = Tool.extend(function() {
			...
			for (var left_ix=0; left_ix<left_keys.length; left_ix++) {
				...
				for (var right_ix=0; right_ix<right_tool_ids.length; right_ix++) {
					...
					if (matching_outputs.length==0) {
						...
						var info = right_hash[right_key];
						var outp = info[0].get_output(info[2]);
						//console.log("disqualifying " + info[1] + ", for socket " + info[2] + " of tool " + info[0]);
	...
	this.num_inputs = function() { return this.id2tool[0].num_inputs(); }

	this.num_outputs = function() { 
		// this should return the same value as this.id2tool[1].num_outputs().
		return this.id2tool[1].num_inputs(); 
	}
	 ...
	 */
	function C_embed(savestatename, bbox) {
		...
		for (var i=0; i<new_ct.max_input_socket(); i++) {
			...
			if (input) {
				assert(input[0]===this.ControlPoints, 
						"Embedded compoundtool has non-controlpoint input?!");
				var my_socket = ii.first_free_input();
				new_ct.disconnect(i);
				new_ct.connect(ii, my_socket, i);
				ii.connect(this.ControlPoints, input[1], my_socket);
			}
		}

		// and unmark all outputs of the embedded tool as outputs (UGLY HACK)
		for (var i=0; i<new_ct.max_output_socket(); i++) {
			var gizmo = new_ct.id2tool[1].listen(i);
			if (gizmo) gizmo.set_class("output", false);
		}
	...
	function C_connect_output(left_tool_id, left_out_socket, right_in_socket) {
		var left_tool = this.id2tool[left_tool_id];
		this.id2tool[1].connect(left_tool, left_out_socket, right_in_socket);
		left_tool.get_output(left_out_socket).set_class("output", true);
	}

	function C_disconnect_output(left_tool_id, left_out_socket, right_in_socket) {
		this.id2tool[1].disconnect(right_in_socket);
		this.id2tool[left_tool_id].get_output(left_out_socket).set_class("output", false);
	}
