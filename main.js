...
/*
	...
	for (var i = 0; i < N_STAMPS; i++) {
	    var stamp_object = this.SVG.create(stamp_margin, (stamp_margin*(i+1)) + (stamp_height*i),
...
*/
	...
	var stamp = LineStamp.create();
	stamp.attach_svg_object(Graphics.SVG.create([10,10,200,150]));
	stamp.redraw();
	
	var stamp2 = CircleStamp.create();
	stamp2.attach_svg_object(Graphics.SVG.create([10,200,200,150]));
	stamp2.redraw();
	...
	var stamp3 = ConstructionStamp.create();
	stamp3.attach_svg_object(Graphics.SVG.create([10,390,200,150]));
	var savestate = Storage.get_file("file_0");
	if (savestate) {
		var stamp_constr = Construction.create();
		stamp_constr.initialize(savestate[1]);
		stamp3.attach_construction(stamp_constr);
	}
	stamp3.redraw();
	...
	window.onkeypress = function(e) {
		...
		if (STATE == "normal") {
			...
			case 122: // Z, undo
				STATE = "animating";
				State.undo(post_animation, MAIN_SVG);
				break;
			case 120: // X, redo
				STATE = "animating";
				State.redo(post_animation, MAIN_SVG);
				break;
			default:
